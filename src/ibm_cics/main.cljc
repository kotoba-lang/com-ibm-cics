(ns ibm_cics.main
  "Kotodama WASM entrypoint for the IBM CICS clean-room actor (L4) — Clojure port.
  ...
  facts (`ibm_cics.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ibm_cics")
(def tier "L4")
...
(def entity-specs
  [{:entity "Job"        :plural "jobs"        :id-prefix "ibmcics_job"
    :fields [:jcl :class_ :owner :status]        :required [:jcl :class_]
    :coerce {}                       :refs {}}
   {:entity "Dataset"    :plural "datasets"    :id-prefix "ibmcics_dat"
    :fields [:name :dsorg :recfm :sizeTracks]    :required [:name :dsorg]
    :coerce {:sizeTracks :int}       :refs {}}
   {:entity "Region"     :plural "regions"     :id-prefix "ibmcics_reg"
    :fields [:name :type :maxTasks]              :required [:name :type]
    :coerce {:maxTasks :int}         :refs {}}
   {:entity "Subsystem"  :plural "subsystems"  :id-prefix "ibmcics_sub"
    :fields [:name :type :status]                :required [:name :type]
    :coerce {}                       :refs {}}
   {:entity "Transaction" :plural "transactions" :id-prefix "ibmcics_tra"
    :fields [:code :program :regionId :status]   :required [:code :program]
    :coerce {}                       :refs {:regionId "Region"}}
   {:entity "Queue"      :plural "queues"      :id-prefix "ibmcics_que"
    :fields [:name :depth :maxDepth]             :required [:name :depth]
    :coerce {:depth :int :maxDepth :int}         :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ibm_cics.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ibm_cics-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ibm_cics-compat/ready)
